...
data:
  ...
  bundledCode: "#line 1 \"poly/test/composition_of_formal_power_series_large.test.cpp\"\
    ...
    \                wq[i] = p[4 * w * i + j];\n            }\n            std::fill(wq.begin()\
    \ + h / 2, wq.end(), M());\n            transposed_ifft(wq);\n            for\
    \ (int i = 0; i < h; ++i) {\n                rp[2 * i] = wq[i] * rq[j][2 * i +\
    \ 1] * INV2 * invs[i];\n                rp[2 * i + 1] = -wq[i] * rq[j][2 * i]\
    \ * INV2 * invs[i];\n            }\n            transposed_fft(rp);\n        \
    \    for (int i = 0; i < h; ++i) {\n                p[2 * w * i + j] = rp[i];\n\
    \            }\n        }\n        for (int i = 0; i < 2 * n; i += 2 * w) {\n\
    \            transposed_fft(p.data() + i + w, w);\n            M pw(1);\n    \
    \        for (int j = i + w; j < i + 2 * w; ++j) {\n                p[j] *= pw;\n\
    \                pw *= omega;\n            }\n            transposed_ifft(p.data()\
    \ + i + w, w);\n            for (int j = i; j < i + w; ++j) {\n              \
    \  p[j] += p[j + w];\n                p[j + w] = M();\n            }\n       \
    \ }\n    };\n    rec(rec, n, 1);\n    std::vector<M> ans(n);\n    for (int i =\
    \ 0; i < n; ++i) {\n        ans[i] = p[2 * i];\n    }\n    reverse(ans.begin(),\
    \ ans.end());\n    return ans;\n}\n\n// f(g(x)), length: max(f.size(), g.size())\n\
    template <typename M>\nstd::vector<M> compose(std::vector<M> f, std::vector<M>\
    \ g) {\n    if (f.empty() || g.empty()) {\n        return std::vector<M>();\n\
    ...
    \      write_char('\\n');\n    }\n};\n\nReader rd(stdin);\nWriter wr(stdout);\n\
    #line 5 \"poly/test/composition_of_formal_power_series_large.test.cpp\"\n\nint\
...
layout: document
redirect_from:
- /verify/poly/test/composition_of_formal_power_series_large.test.cpp
- /verify/poly/test/composition_of_formal_power_series_large.test.cpp.html
