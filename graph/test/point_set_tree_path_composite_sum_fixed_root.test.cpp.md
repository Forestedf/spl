...
data:
  ...
  bundledCode: "#line 1 \"graph/test/point_set_tree_path_composite_sum_fixed_root.test.cpp\"\
    ...
    \ T, bool DIR>\n    DynamicTreeDP(Graph<T, DIR> &g, TreeDP &dp, int root = 0)\
    \ : dp(dp), stt(g, root), value(stt.nodes.size()), func(stt.nodes.size()) {\n\
    \        init(stt.stt_root);\n    }\n    Value update(int v) {\n        while\
    \ (v != -1) {\n            update_single(v);\n            v = stt.nodes[v].par;\n\
    ...
    \        val.second += M::raw(1);\n        val.first = fs[vtx].first * val.first\
    \ + fs[vtx].second * val.second;\n        return Func(fs[vtx].first, fs[vtx].second,\
  ...
  verifiedWith: []
documentation_of: graph/test/point_set_tree_path_composite_sum_fixed_root.test.cpp
layout: document
redirect_from:
- /verify/graph/test/point_set_tree_path_composite_sum_fixed_root.test.cpp
- /verify/graph/test/point_set_tree_path_composite_sum_fixed_root.test.cpp.html
